...
class PlaylistsRepositoryImpl(
    ...
    override suspend fun addPlaylist(playlist: PlaylistData): Long {
        val entity = PlaylistEntity(
            id = if (playlist.id == 0L) 0 else playlist.id, // 0 для автогенерации
            name = playlist.name,
        ...
        return dao.insert(entity) // Room возвращает ID вставленной записи
    }

    override suspend fun getPlaylists(): List<PlaylistData> {
        return dao.getAllPlaylists().first().map { entity ->
            PlaylistData(
                id = entity.id,
                name = entity.name,
